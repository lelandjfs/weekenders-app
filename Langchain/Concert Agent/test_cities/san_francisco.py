"""
San Francisco Concert Test
===========================

Test Concert Agent for SF Bay Area to verify:
- "too_small" classification → expansion to Oakland, Berkeley, etc.
- All sources working (Ticketmaster, Songkick, Bandsintown)
- Geographic expansion beyond just SF proper
"""
...
def test_san_francisco():
    """Test SF Bay Area concerts - should expand to Oakland, Berkeley, etc."""
    ...
    print("\n" + "="*70)
    print("🌉 SAN FRANCISCO BAY AREA CONCERT TEST")
    print("="*70)
    print(f"\n📅 TODAY: December 1, 2025")
    print(f"📅 Weekend: {start_date} to {end_date} (Fri-Sun)")
    print(f"📍 Location: {location}")
    print(f"\n🎯 Goals:")
    print(f"   • Test 'too_small' classification")
    print(f"   • Verify expansion to Oakland, Berkeley, etc.")
    print(f"   • Test all data sources working together")
    print("="*70)
    ...
    if context.neighborhoods:
        print(f"\n   🏘️  Expanded Cities ({len(context.neighborhoods)}):")
    ...
    print(f"     • Songkick: {songkick_count}")
    print(f"     • Bandsintown: {bandsintown_count}")
    print(f"     • Web (generic): {web_count}")

    # Geographic diversity - check for Oakland, Berkeley!
    if final_results.concerts:
        ...
        # Check specifically for Bay Area cities
        sf_concerts = [c for c in final_results.concerts if c.location and "San Francisco" in c.location]
        oakland_concerts = [c for c in final_results.concerts if c.location and "Oakland" in c.location]
        berkeley_concerts = [c for c in final_results.concerts if c.location and "Berkeley" in c.location]
        other_concerts = [c for c in final_results.concerts if c.location and
                         "San Francisco" not in c.location and
                         "Oakland" not in c.location and
                         "Berkeley" not in c.location]

        print(f"\n   📍 City Breakdown:")
        print(f"      • San Francisco: {len(sf_concerts)} concerts")
        print(f"      • Oakland: {len(oakland_concerts)} concerts ← Should expand here!")
        print(f"      • Berkeley: {len(berkeley_concerts)} concerts ← Should expand here!")
        print(f"      • Other Bay Area: {len(other_concerts)} concerts")
    ...
    with open(output_dir / "metadata.json", "w") as f:
        json.dump({
            ...
            "city_breakdown": {
                "san_francisco": len(sf_concerts) if final_results.concerts else 0,
                "oakland": len(oakland_concerts) if final_results.concerts else 0,
                "berkeley": len(berkeley_concerts) if final_results.concerts else 0,
                "other_bay_area": len(other_concerts) if final_results.concerts else 0
            }
        }, f, indent=2)

    print(f"\n💾 Saved to: {output_dir}")
    print(f"   Structure: San_Francisco_CA/run_timestamp/weekend_dates/files")
    print("="*70 + "\n")

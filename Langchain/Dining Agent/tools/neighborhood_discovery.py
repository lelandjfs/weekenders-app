"""
Neighborhood Discovery Tool for LangChain
==========================================

LangChain-compatible tool for discovering trendy food neighborhoods.
Uses Claude Haiku to extract neighborhood names from Reddit and web search.
"""
...
from config import TAVILY_API_KEY, ANTHROPIC_API_KEY, MAX_NEIGHBORHOODS


class NeighborhoodDiscoveryInput(BaseModel):
    """Input schema for neighborhood discovery."""
    city: str = Field(description="City name to discover neighborhoods for")
    max_neighborhoods: int = Field(
        default=5,
...
def discover_neighborhoods(city: str, max_neighborhoods: int = 5) -> List[str]:
    ...
    Args:
        ...
        max_neighborhoods: Maximum neighborhoods to return
